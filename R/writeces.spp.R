...
function(x, file="", age=0){
 ...
 cesdata<-unique(cbind(x$spp,x$ring,x$year)); colnames(cesdata)=c('spp','ring','year')
 cesnames<-cesnames[ , c(1,which(colnames(cesnames)==lang)) ]
 cesdata<-merge(cesdata,cesnames,all.x=T); colnames(cesdata)[4]='species'
 ...
 capture.output(xtabs(~species+year,data=cesdata),file=file,append=T)
